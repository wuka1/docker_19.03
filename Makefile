...
build: bundles
	...
	DOCKER_BUILDKIT="${DOCKER_BUILDKIT}" docker build --network host --build-arg=GO_VERSION ${BUILD_APT_MIRROR} ${DOCKER_BUILD_ARGS} ${DOCKER_BUILD_OPTS} -t "$(DOCKER_IMAGE)" -f "$(DOCKERFILE)" .
